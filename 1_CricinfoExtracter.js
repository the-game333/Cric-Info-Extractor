...
reponseKaPromise.then(function (response) {
    ...
    let matches = [];
    let matchdivs = document.querySelectorAll("div.ds-px-4.ds-py-3");
    ...
    for (let i = 0; i < matchdivs.length; i++) {
        ...
        let teamParas = matchdiv.querySelectorAll("p.ds-text-tight-m.ds-font-bold.ds-capitalize");
        match.t1 = teamParas[0].textContent;
        match.t2 = teamParas[1].textContent;

        let scoreSpans = matchdiv.querySelectorAll("div.ds-text-compact-s.ds-text-typo-title");
        ...
        let resultSpan = matchdiv.querySelector("p.ds-text-tight-s.ds-font-regular.ds-truncate.ds-text-typo-title");
